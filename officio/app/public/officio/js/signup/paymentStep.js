...
function sendCCInfo() {
    ...
    $.ajax({
        type: 'POST',
        url: baseUrl + '/signup/index/payment-submit',
        ...
        success: function (response) {
            if (!response.success) {
                ...
                if(response.pkey){
                    // Don't show submit button again
                } else {
                    $('#nextBtn').show();
                }
                ...
                $('#previousBtn').addClass('disabled').hide();
                $('#confirm').addClass('active');
                $('.wizardcontent').hide();
                $('#success_message').show();
                $("#successContinueBtn").attr("href", baseUrl + '/companywizard?key=' + response.pkey + '&step=3');

                var trn = response.transaction;

                if (typeof googleTagManagerPurchase !== 'undefined') {
                    googleTagManagerPurchase(
                        trn.transaction_id,
                        trn.amount,
                        0,
                        trn.currency,
...
$(document).ready(function () {
    ...
    jQuery.validator.addMethod('checkState', function (val, element) {
        return $('#country').val() != CaCountryId || $('#state-div').css('display') == 'none' || val != '';
    }, _('Please enter a valid ' + stateLocalName + '.'));

    jQuery.validator.addMethod('checkStateId', function (val, element) {
        return $('#country').val() != CaCountryId || $('#state-id-div').css('display') == 'none' || val != '';
    }, _('Please select a ' + stateLocalName + '.'));
    ...
    jQuery.validator.addMethod('postalUniques', function (postal, element) {
        ...
        return $('#country').val() != CaCountryId || postal.match(/^[0-9A-Za-z]+([\s\-]{1}[0-9A-Za-z]+)?$/i);
     ...
     * Pricing summary
     */
    function calculateSignupPricing(){

        var price = 0;
        var payment_term = $('input[type="radio"][name="payment_term"]:checked').val();
        if(payment_term === '1'){
            price = parseFloat(priceMonth);
            $('#annual-savings-info').show();
        } else {
            price = parseFloat(priceAnnually);
            $('#annual-savings-info').hide();
        }

        var country = $('#country').val();
        if(defaultCountryId == AuCountryId && country == AuCountryId){

            // AU, hardcode 10%

            var gst = price * 0.1;
            $('#signup_pricing_gst').html(gst.toFixed(2));

            var total = price + gst;
            $('#signup_pricing_total').html(total.toFixed(2));

            $('.summary-gst').show();
            $('.summary-gst-unknown').hide();

        } else if(defaultCountryId == CaCountryId && country == CaCountryId){

            // CA, calculate per province

            var stateId = $('#stateId').val();
            if(stateId && arrStatesTaxCA.hasOwnProperty(stateId)){

                var gstRate = parseFloat(arrStatesTaxCA[stateId].rate) / 100;
                var gst = price * gstRate;
                $('#signup_pricing_gst').html(gst.toFixed(2));
    ...
    $('input[type="radio"][name="payment_term"]').click(function(){
    ...
    // Initial run
    $('input[type="radio"][name="payment_term"]').first().prop("checked", true);
    calculateSignupPricing();
});
