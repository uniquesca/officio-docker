var ApplicantsQueueLeftSectionGrid = function (config, owner) {
    this.owner = owner;
    ...
    var genId = Ext.id();
    ApplicantsQueueLeftSectionGrid.superclass.constructor.call(this, {
        id:               genId,
        sm:               sm,
...
Ext.extend(ApplicantsQueueLeftSectionGrid, Ext.grid.GridPanel, {
    ...
    onSelectionChange: function (sm) {
        ...
        if (arrSelected.length) {
            var mainPanel = Ext.getCmp(this.panelType + '-panel');
            if (mainPanel) {
                var sm2 = mainPanel.FavoriteSearchesPanel.ApplicantsSearchFavoriteGrid.getSelectionModel();
                sm2.suspendEvents();
                sm2.clearSelections();
                sm2.resumeEvents();
            }
            ...
            if (tabPanel) {
                // Save to cookies
                Ext.state.Manager.set('checked_offices', arrSelected);
                tabPanel.openQueueTab(arrSelected);
            }
        }
    },
    ...
    preselectDefaultOffices: function (store, records) {
        ...
        if (this.arrQueuesToShow.length) {
            ...
            arrDefaultRecords = Ext.getCmp(this.panelType + '-panel').getDefaultOffices();
        }
