...
Ext.ux.form.LovCombo = Ext.extend(Ext.form.ComboBox, {
    ...
    /**
     * @cfg {Boolean} A custom behaviour if one option is selected, and we select/click the same or another option
     */
    , customBehaviourForOneOption: false
    ...
    ,getCheckedValue:function(field) {
        ...
        // don't add 'Select All' to the list
        if (this.useSelectAll && field==this.displayField && c.length==this.store.data.length)
            c.shift();
    ...
    ,onSelect:function(record, index) {
        if(this.fireEvent('beforeselect', this, record, index) !== false){
            // index==0 => check/uncheck all
            if (this.useSelectAll && index==0)
            {
            ...
            // uncheck 'All' checkbox, if at least one checkbox is unchecked
            if (this.useSelectAll && record.data.checked)
                this.store.getAt(0).set(this.checkField, false);
    ...
    // {{{
    /**
     * Sets the value of the LovCombo. The passed value can be a falsie (null, false, empty string), in
    ...
    , onViewClick : function(doFocus, p1, p2, e){
        var index = this.view.getSelectedIndexes()[0],
            s = this.store,
            r = s.getAt(index);

        // A custom behaviour if one option is checked only, and we select the same or another option
        if (this.customBehaviourForOneOption && !Ext.get(e.target).hasClass('ux-lovcombo-icon')) {
            var checked = this.getCheckedValue().split(this.separator);
            if (checked.length === 1 && checked[0] != r.get(this.valueField)) {
                this.deselectAll();
            }
        }

        // Here is the same code as in the Ext.form.ComboBox
        if(r){
