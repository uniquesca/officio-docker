<table cellpadding="0" cellspacing="0" border="0" class="settings">
        ...
        <td valign="top">
            <div class="backgrounds cws-items-block">
                ...
                <div class="cws-item<?= (empty($selectedBg) ? ' active' : '') ?>" style="float: left; padding: 5px;">
                    ...
                    <label class="cws-template-bg" for="selected-bg-default"><span style="background: url('<?= $this->templateUrl . '/images/bg.gif' ?>') repeat;" alt=""></span></label>
                </div>
    ...
    <tr>
        <?php $footerBg = $this->options['footer-bg'] ?? false; ?>
        <td width="170"><label>Footer background color:</label></td>
        <td>
            <?php foreach (array('#61584F', '#3A342D') as $key => $color) { ?>
                <div class="cws-color-item" style="background-color: <?= $color ?>;">
                    <input type="radio" name="footer-bg" value="<?= $color ?>" <?= ((empty($footerBg) && $key == 0) || $footerBg == $color ? ' checked="checked"' : '') ?> />
                </div>
            <?php } ?>
        </td>
    </tr>
