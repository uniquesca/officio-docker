...
class ServerExceptions implements ServerExceptionsInterface
{
    ...
    public function getExceptionMessageByCode($strCode, $strExceptionMessage, $strSection)
    {
        switch ($strCode) {
            case '00':
            case '08':
            ...
            case 54:
                $strExceptionMessage = PAYMENT_SERVICE_PROFILE_DATE_STATUS;
                break;

            case 'QE':
                if ($strSection == 'profile_delete') {
                    $strExceptionMessage = PAYMENT_SERVICE_PROFILE_UPDATE_STATUS;
                } else {
                    $strExceptionMessage = empty($strExceptionMessage) ? COMPLEX_ERROR : $strExceptionMessage;
                }
                break;
