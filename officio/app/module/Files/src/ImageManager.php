...
class ImageManager
{
    ...
    private $image_library = 'gd2';    // Can be:  imagemagick, netpbm, gd, gd2
    private $library_path = '';
    private $dynamic_output = false;    // Whether to send to browser or write to disk
    ...
    private $master_dim = 'auto';    // auto, height, or width.  Determines what to use as the master dimension
    private $rotation_angle = '';
    private $x_axis = '';
    private $y_axis = '';

    // Watermark Vars
    private $wm_text = '';            // Watermark text if graphic is not used
    private $wm_type = 'text';        // Type of watermarking.  Options:  text/overlay
    private $wm_x_transp = 4;
    private $wm_y_transp = 4;
    private $wm_overlay_path = '';            // Watermark image path
    private $wm_font_path = '';            // TT font
    private $wm_font_size = 17;            // Font size (different versions of GD will either use points or pixels)
    private $wm_vrt_alignment = 'B';            // Vertical alignment:   T M B
    private $wm_hor_alignment = 'C';            // Horizontal alignment: L R C
    private $wm_padding = 0;            // Padding around text
    private $wm_hor_offset = 0;            // Lets you push text to the right
    private $wm_vrt_offset = 0;             // Lets you push  text down
    private $wm_font_color = '#ffffff';    // Text color
    private $wm_shadow_color = '';            // Dropshadow color
    private $wm_shadow_distance = 2;            // Dropshadow distance
    private $wm_opacity = 50;            // Image opacity: 1 - 100  Only works with image
     ...
     * @param array $props
     */
    public function __construct(TranslatorInterface $translator, $props = array())
    {
    ...
    public function text_watermark()
    {
        ...
        if ($this->wm_use_truetype and !file_exists($this->wm_font_path)) {
        ...
        if ($this->wm_use_truetype) {
            ...
            $this->wm_vrt_offset += (int)$this->wm_font_size;
        } else {
        ...
        switch ($image_type) {
            case     1 :
                if (!function_exists('imagecreatefromgif')) {
                    $this->set_error(
                        array('Your server does not support the GD function required to process this type of image.', 'GIF images are often not supported due to licensing restrictions.  You may have to use JPG or PNG images instead.')
                    );
                    return false;
                }
     ...
     * TODO PHP7 Should we rework this?
     * @access    public
     * @param resource $resource
     * @return    void
     */
     ...
     * @return    array|bool
     */
    public function get_image_properties($path = '', bool $return = false)
    {
     ...
     * @return array
     * TODO Consider replacing usage with pathinfo
     */
    ...
    {
        if (is_array($msg)) {
            foreach ($msg as $errorDetail) {
                $this->error_msg[] = $this->_translator->translate($errorDetail);
            }
        } else {
            $this->error_msg[] = $this->_translator->translate($msg);
        }
    }
