...
class AddCaseTypeFieldTypeAndEmptyTopGroups extends AbstractMigration
{
    ...
    public function up()
    {
        $this->execute("UPDATE `client_form_fields` SET `label`='Immigration Program (legacy field)' WHERE company_field_id = 'immigration_program'");
        ...
        foreach ($arrAllCompaniesRoles as $arrCompanyRole) {
            ...
            if (isset($arrFieldMapping[$arrCompanyRole['company_id']])) {
                $arrFieldsAccess[] = sprintf(
                    ...
                    in_array($arrCompanyRole['role_type'], ['individual_client', 'employer_client']) ? 'R' : 'F'
                );
            }
        }
