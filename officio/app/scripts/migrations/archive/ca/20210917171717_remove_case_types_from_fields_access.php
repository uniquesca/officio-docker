...
class RemoveCaseTypesFromFieldsAccess extends AbstractMigration
{
    ...
    public function down()
    {
        ...
        foreach ($arrClientTypes as $arrClientTypeInfo) {
            $arrClientTypesGrouped[$arrClientTypeInfo['company_id']] = $arrClientTypeInfo['client_type_id'];
        }

        $statement = $this->getQueryBuilder()
            ->select(['a.*', 'f.company_id'])
            ->from(['a' => 'client_form_field_access'])
            ->leftJoin(['f' => 'client_form_fields'], ['a.field_id = f.field_id'])
        ...
        foreach ($arrFields as $arrFieldInfo) {
            if (isset($arrClientTypesGrouped[$arrFieldInfo['company_id']])) {
                $this->getQueryBuilder()
                    ->update('client_form_field_access')
                    ->set(['client_type_id' => $arrClientTypesGrouped[$arrFieldInfo['company_id']]])
