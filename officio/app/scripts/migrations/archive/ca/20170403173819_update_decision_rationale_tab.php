...
class UpdateDecisionRationaleTab extends AbstractMigration
{
    public function up()
    {
        ...
        $this->execute("ALTER TABLE `u_notes` ADD COLUMN `type` ENUM('general','draft') NOT NULL DEFAULT 'general' AFTER `note_color`;");
    }
    ...
    public function down()
    {
        ...
        $this->execute("ALTER TABLE `u_notes` ADD COLUMN `field_id` INT(11) UNSIGNED DEFAULT NULL AFTER `note_color`;");

        $this->execute("ALTER TABLE `u_notes` ADD CONSTRAINT `FK_u_notes_client_form_fields` FOREIGN KEY (`field_id`) REFERENCES `client_form_fields` (`field_id`) ON UPDATE CASCADE ON DELETE CASCADE;");
    }
}