...
class ChangeCountryForVisaSurvey extends AbstractMigration
{
    ...
    public function down()
    {
        try {
            ...
            $this->execute('ALTER TABLE `client_form_dependents_visa_survey` ADD CONSTRAINT `FK_client_form_dependents_visa_survey_country_master` FOREIGN KEY (`visa_country_id`) REFERENCES `country_master` (`countries_id`) ON UPDATE CASCADE ON DELETE CASCADE;');
        } catch (\Exception $e) {
            /** @var Log $log */
            $log = self::getService('log');
            $log->debugErrorToFile($e->getMessage(), $e->getTraceAsString());
