...
class UpdateMainTables extends AbstractMigration
{
    public function up()
    {
        ...
        $this->execute("UPDATE `company_details` SET `employers_module_enabled`='Y' WHERE subscription IN ('ultimate_plus', 'ultimate') OR company_id = 0;");
        ...
        $this->execute("INSERT INTO `client_types` (`company_id`, `client_type_name`) SELECT c.company_id, 'Generic' FROM company as c;");
        ...
        $this->execute("UPDATE `members` SET `login_enabled`='N' WHERE userType IN (3, 7, 8, 9, 10) AND (username = '' OR username IS NULL);");
