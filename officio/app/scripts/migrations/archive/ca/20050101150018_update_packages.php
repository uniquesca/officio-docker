...
class UpdatePackages extends AbstractMigration
{
    public function up()
    {
        ...
        $this->execute("INSERT INTO `acl_rule_details` (`rule_id`, `module_id`, `resource_id`, `resource_privilege`) VALUES (11, 'applicants', 'profile', 'upgrade-subscription-plan');");
        ...
        // Rename incorrectly named roles (must be only 1 Client role)
        $this->execute("UPDATE `acl_roles` SET `role_name`='Client' WHERE `role_id` IN (10, 3400, 3562, 8557);");
        $this->execute("UPDATE `acl_roles` SET `role_name`='User' WHERE `role_id` IN (2920);");
        ...
        $this->execute("ALTER TABLE `company_details` CHANGE COLUMN `client_file_number_settings` `case_number_settings` TEXT NULL COMMENT 'Case number generation settings' AFTER `marketplace_module_enabled`;");
