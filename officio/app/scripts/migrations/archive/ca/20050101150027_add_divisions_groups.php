...
class AddDivisionsGroups extends AbstractMigration
{
    public function up()
    {
        // Took 194s on the local server
        $this->execute("CREATE TABLE IF NOT EXISTS `divisions_groups` (
        ...
        foreach ($arrCompanies as $companyId) {
            ...
            $statement = $this->getQueryBuilder()
                ...
                ->into('divisions_groups')
                ->values($arrInsert)
                ->execute();

            $groupId = $statement->lastInsertId('divisions_groups');
