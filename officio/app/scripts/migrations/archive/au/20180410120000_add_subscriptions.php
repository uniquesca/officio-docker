...
class AddSubscriptions extends AbstractMigration
{
    public function up()
    {
        ...
        // The difference is:
        // - Names are different
        // - Pro (pack 1 + 3) is not visible for AU
        if (Zend_Registry::get('serviceManager')->get('config')['site_version']['version'] === 'australia') {
            $this->execute(
                "INSERT INTO `subscriptions` (`subscription_id`, `subscription_name`, `subscription_hidden`, `subscription_order`) VALUES
                ('lite', 'Solo', 'N', 1),
                ('pro', 'Professional', 'N', 2),
                ('pro13', 'Professional (with Pack 1 and 3)', 'Y', 3),
                ('ultimate', 'Ultimate', 'N', 4),
                ('ultimate_plus', 'Ultimate Plus', 'Y', 5);"
            );
        } else {
            $this->execute(
                "INSERT INTO `subscriptions` (`subscription_id`, `subscription_name`, `subscription_hidden`, `subscription_order`) VALUES
