...
class AddHtmlConTemplate extends AbstractMigration
{
    public function up()
    {
        try {
            ...
            $db->insert(
                ...
                array(
                    'type'        => 'system',
                    'title'       => 'Certificate of Naturalisation (for HTML preview)',
                    'subject'     => 'Certificate of Naturalisation (for HTML preview)',
                    'template'    => $template,
                    'create_date' => date('Y-m-d'),
                )
            );

            $db->query("INSERT INTO `acl_rule_details` (`rule_id`, `module_id`, `resource_id`, `resource_privilege`, `rule_allow`)
                                SELECT `rule_id`, 'applicants', 'profile', 'export-con', 1
                                FROM acl_rule_details AS d
                                WHERE d.resource_privilege = 'generate-con'");
            ...
            <table border="0" cellpadding="2" align="left" width="100%" style="font-size: 10px;">
             <tr>
              <td width="32%"></td>
            ...
            $db->update(
                'system_templates',
                array(
                    'title'    => 'Certificate of Naturalisation (for PDF generation)',
                    'subject'  => 'Certificate of Naturalisation (for PDF generation)',
                    'template' => $template
                ),
    ...
    public function down()
    {
        try {
            ...
            $db->delete(
                'acl_rule_details',
                $db->quoteInto('resource_privilege = ?', 'export-con')
            );
            ...
            $template = '<table border="0" cellpadding="3" align="left" width="100%">
                ...
                <h1 align="center">CERTIFICATE OF NATURALISATION</h1>
                <p style="text-indent: 30px;">WHEREAS <i><b>{main_applicant_name}</b></i> has applied to the Minister responsible for citizenship in the Government of Dominica for a certificate of naturalisation, alleging with respect to <input type="text" name="self_{page_number}" size="7" value="{self_name}" /> the particulars set out below, and has satisfied the Minister that the conditions laid down in the Constitution and in the Citizenship Act Chapter 1:10 for the grant of a certificate of naturalisation are fulfilled:</p>
                ...
                <table border="0" cellpadding="2" align="left" width="83%" style="font-size: 10px;">
                 ...
                 <tr>
                  <td width="37%" height="30px"></td>
                  <td width="30%" height="30px">NAME OF SPOUSE:</td>
                  <td width="33%" height="30px"><textarea name="name_of_spouse_{page_number}" cols="37" rows="2">{name_of_spouse}</textarea></td>
                 </tr>
                </table>';
