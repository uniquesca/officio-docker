    ...
    public function up()
    {
        ...
        try {
            ...
            $select = $db->select()
                ...
                ->joinInner(array("f" => "applicant_form_fields"), "d.applicant_field_id = f.applicant_field_id", "")
                ->where("f.encrypted = ?", "Y")
                ->where("d.value IS NOT NULL AND TRIM(d.value) <> ''");

            $arrValues = $db->fetchAll($select);
            ...
            foreach ($arrValues as $arrValueInfo) {
                ...
                if ($decoded) {
                    ...
                    $arrWhere = array(
                        $db->quoteInto('applicant_id = ?', $arrValueInfo['applicant_id'], 'INT'),
                        $db->quoteInto('applicant_field_id = ?', $arrValueInfo['applicant_field_id'], 'INT'),
                        $db->quoteInto('row = ?', $arrValueInfo['row'], 'INT')
                    );

                    if (!empty($arrValueInfo['row_id'])) {
                        $arrWhere[] = $db->quoteInto('row_id = ?', $arrValueInfo['row_id']);
                    }

                    $db->update(
                        'applicant_form_data',
                        array('value' => $reencryptedValue),
