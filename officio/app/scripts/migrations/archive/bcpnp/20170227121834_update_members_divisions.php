...
class UpdateMembersDivisions extends AbstractMigration
{
    public function up()
    {
        $this->execute("ALTER TABLE `members_divisions`
            ADD COLUMN `type` ENUM('access_to','responsible_for','pull_from') NULL DEFAULT 'access_to' AFTER `division_id`;
        ");

        $this->execute("UPDATE `members_divisions` SET `type`='responsible_for' WHERE `responsible_for`='Y';");
