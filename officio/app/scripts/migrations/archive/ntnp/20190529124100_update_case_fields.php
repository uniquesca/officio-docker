...
class UpdateCaseFields extends AbstractMigration
{
...

    {
        ...
        try {
            ...
            foreach ($arrCompanyIds as $companyId) {
                ...
                //
                //                list($strError, $arrFieldsAdded[]) = $oClients->getApplicantFields()->saveField($companyId, $internalContactGroupId, 0, 0, $oFieldTypes->getFieldTypeId($arrFieldsInsert['type']), $applicantFieldId, $arrFieldsInsert, array(), false, 0, 0, false, '', false, false, false);
                //
                //                $arrFieldsInsert = array(
                //                    'member_type_id' => 9,
                //                    'type' => 'text',
                ...
                //                list($strError, $arrFieldsAdded[]) = $oClients->getApplicantFields()->saveField($companyId, $internalContactGroupId, 0, 0, $oFieldTypes->getFieldTypeId($arrFieldsInsert['type']), $applicantFieldId, $arrFieldsInsert, array(), false, 0, 0, false, '', false, false, false);
                //
                //                $groupId = $oClients->getApplicantFields()->createGroup($companyId, 8, $blockId, 'Spouse Information', 'N', 3);
                //
                //                $oClients->getApplicantFields()->toggleContactFields($companyId, 8, $blockId, $groupId, $arrFieldsAdded, array());
                //                foreach ($arrFieldsAdded as $fieldId) {
                //                    $oClients->getApplicantFields()->allowFieldAccessForCompanyAdmin($companyId, $fieldId, $groupId);
                ...
                $fieldOptions = array(
                    ...
                    array(
                        'name'  => 'BNP- Pre-interview Info Submitted',
                        'order' => 3
                    ),
                    array(
                    ...
                    array(
                        'name'  => 'BNP- Good Faith Deposit Submitted',
                        'order' => 13
                    ),
                    array(
                ...
                $arrFieldsInsert = array(
                    'type'      => $oFieldTypes->getFieldTypeId('date'),
                    'label'     => 'Work Permit Application Deadline',
                    'required'  => 'N',
                    'encrypted' => 'N'
                );
                $fieldCompanyId  = 'work_permit_application_deadline';
                list($strError, $arrFieldsAdded['work_permit_application_deadline']) = $oClients->getFields()->saveField(
                    $companyId,
                    0,
                    0,
                    0,
                ...
                foreach ($arrCaseTypes as $templateId) {
                    ...
                    if ($templateId === $businessTemplateId) {
                        ...
                        $arrGroupFields[] = $arrFieldsAdded['work_permit_application_deadline'];
                    ...
                    if ($templateId === $businessTemplateId) {
                        ...
                        $arrGroupFields[] = $arrFieldsAdded['explanation_for_denial'];
                    }

                    foreach ($arrGroupFields as $updateFieldId) {
                        $fullRow = in_array($updateFieldId, array($arrFieldsAdded['accompanying_dependent_details'], $arrFieldsAdded['explanation_for_denial'])) ? 'Y' : 'N';
