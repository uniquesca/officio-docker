...
class AddDocumentsChecklist extends AbstractMigration
{
    ...
    public function down()
    {
        try {
            ...
            $db->delete(
                'acl_rules',
                $db->quoteInto('rule_check_id = ?', 'client-documents-checklist-view')
            );

            $db->query('DROP TABLE IF EXISTS `client_form_dependents_uploaded_file_tags`;');
            $db->query('DROP TABLE IF EXISTS `client_form_dependents_uploaded_files`;');
            $db->query('DROP TABLE IF EXISTS `client_form_dependents_required_files`;');
