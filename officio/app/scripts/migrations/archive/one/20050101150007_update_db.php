...
class UpdateDb extends AbstractMigration
{
    public function up()
    {
        ...
        $this->execute("CREATE TABLE `client_types_kinds` (
        	...
        	`client_type_kind` ENUM('individual_client','corporate_client') NOT NULL DEFAULT 'individual_client',
        ...
        INSERT INTO `client_types_kinds` (client_type_id, client_type_kind)
        SELECT client_type_id, 'individual_client' FROM client_types;");


        $this->execute("INSERT INTO `acl_rule_details` VALUES (10, 'clients', 'index', 'duplicate', 1);");

        $this->execute("ALTER TABLE `clients` ADD COLUMN `client_type_kind` ENUM('individual_client', 'corporate_client') NOT NULL DEFAULT 'individual_client' AFTER `client_type_id`;");
        ...
        $this->execute("DELETE FROM `acl_role_access` WHERE  `rule_id`=2204;");
        ...
        $this->execute("UPDATE `acl_rules` SET `rule_visible`=1 WHERE  `rule_id`=2204;");
        ...
        $this->execute("DROP TABLE IF EXISTS `client_types_kinds`;");
        $this->execute("CREATE TABLE `client_types_kinds` (
        	`client_type_id` INT(11) UNSIGNED NOT NULL,
        ...
        $this->execute("ALTER TABLE `clients` DROP COLUMN `client_type_kind`;");
