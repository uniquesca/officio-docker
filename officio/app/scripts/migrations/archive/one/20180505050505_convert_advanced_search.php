...
class ConvertAdvancedSearch extends AbstractMigration
{
    ...
    private function getOptionIdByName($companyId, $fieldId, $clientType, $fieldOptionLabel)
    {
        ...
        if ($clientType == 'case') {
            if (!isset($this->caseFieldsOptions[$companyId][$fieldId])) {
                $this->caseFieldsOptions[$companyId][$fieldId] = $oClients->getFields()->getFieldValueByCompanyFieldId($fieldId, $companyId);
            }
            $arrFieldOptions = $this->caseFieldsOptions[$companyId][$fieldId];
        } else {
            if (!isset($this->clientFieldsOptions[$companyId][$fieldId])) {
                $this->clientFieldsOptions[$companyId][$fieldId] = $oClients->getApplicantFields()->getFieldValueByCompanyFieldId($fieldId, $companyId);
            }
    ...
    public function up()
    {
        try {
            ...
            $arrSearches = $db->fetchAll($select);
            ...
            foreach ($arrSearches as $arrSearchInfo) {
                ...
                $query = sprintf(
                    'UPDATE `searches` SET %s, %s WHERE `search_id` = %d',
                    $db->quoteInto('query = ?', Json::encode($arrNewQuery)),
                    $db->quoteInto('columns = ?', Json::encode($arrNewColumns)),
