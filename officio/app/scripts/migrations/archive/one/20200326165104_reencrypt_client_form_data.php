    ...
    public function up()
    {
        ...
        try {
            ...
            $select = $db->select()
                ...
                ->joinInner(array("f" => "client_form_fields"), "d.field_id = f.field_id", "")
                ->where("f.encrypted = ?", "Y")
            ...
            foreach ($arrValues as $arrValueInfo) {
                ...
                if ($decoded) {
                    ...
                    $arrWhere = array(
                        $db->quoteInto('member_id = ?', $arrValueInfo['member_id'], 'INT'),
                        $db->quoteInto('field_id = ?', $arrValueInfo['field_id'], 'INT')
                    );

                    $db->update('client_form_data', array('value' => $reencryptedValue), implode(' AND ', $arrWhere));
                } else {
                   /** @var Log $log */
            $log = Zend_Registry::get('serviceManager')->get('log');
            $log->debugToFile(
