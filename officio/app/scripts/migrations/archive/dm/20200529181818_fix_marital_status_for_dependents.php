...
class FixMaritalStatusForDependents extends AbstractMigration
{
    public function up()
    {
        $this->execute("UPDATE `applicant_form_data` SET `value`='474' WHERE `applicant_field_id`=4609 AND `value`='513'");
        $this->execute("DELETE FROM `applicant_form_default` WHERE  `applicant_form_default_id`=513;");
        $this->execute(
            "ALTER TABLE `client_form_dependents` CHANGE COLUMN `marital_status` `marital_status` ENUM('single','married','engaged','widowed','separated','divorced') NULL DEFAULT NULL COLLATE 'utf8_general_ci' AFTER `profession`;"
        );
        ...
        try {
            ...
            $template = '<div style="font-size: 17px; border: 1px solid #ccc; padding: 10px 10px 40px 10px; margin: 5px 5px 30px 5px; background-color: white;">
                ...
                <p style="text-indent: 30px; padding-bottom: 15px;">
                    WHEREAS <i><b><input type="text" name="main_applicant_name_{page_number}" size="30" value="{main_applicant_name}"/></b></i> has applied to the Minister for National Security in the Government of Dominica for a certificate of naturalisation, alleging with respect to <input type="text" name="self_name_{page_number}" size="30" value="{self_name}"/> the particulars set out below, and has satisfied the Minister that
    ...
    public function down()
    {
        ...
        $this->execute("ALTER TABLE `client_form_dependents` CHANGE COLUMN `marital_status` `marital_status` ENUM('single','married','divorced','widowed') NULL DEFAULT NULL COLLATE 'utf8_general_ci' AFTER `profession`;");
    }
}