...
class ChangeCaseCategoriesStructure extends AbstractMigration
{
    ...
    public function up()
    {
        ...
        $arrClientTypesGrouped = [];
        foreach ($arrClientTypes as $arrClientTypeInfo) {
            $arrClientTypesGrouped[$arrClientTypeInfo['client_type_name']] = $arrClientTypeInfo['client_type_id'];
        }

        $this->execute(sprintf("UPDATE `client_categories` SET `client_type_id`=%d WHERE company_id = 1 AND client_category_abbreviation = 'BCSA'", $arrClientTypesGrouped['Skills Immigration Application']));
        $this->execute(sprintf("UPDATE `client_categories` SET `client_type_id`=%d WHERE company_id = 1 AND client_category_abbreviation = 'BCER'", $arrClientTypesGrouped['Business Immigration Registration']));
        $this->execute(sprintf("UPDATE `client_categories` SET `client_type_id`=%d WHERE company_id = 1 AND client_category_abbreviation = 'BCSR'", $arrClientTypesGrouped['Skills Immigration Registration']));
        $this->execute(sprintf("UPDATE `client_categories` SET `client_type_id`=%d WHERE company_id = 1 AND client_category_abbreviation = 'BCEA'", $arrClientTypesGrouped['Business Immigration Application']));
        $this->execute(sprintf("UPDATE `client_categories` SET `client_type_id`=%d WHERE company_id = 1 AND client_category_abbreviation = 'BCSRR'", $arrClientTypesGrouped['Review Request']));
        $this->execute(sprintf("UPDATE `client_categories` SET `client_type_id`=%d WHERE company_id = 1 AND client_category_abbreviation = 'BCERR'", $arrClientTypesGrouped['Review Request']));
        $this->execute(sprintf("UPDATE `client_categories` SET `client_type_id`=%d WHERE company_id = 1 AND client_category_abbreviation = 'BCEKS'", $arrClientTypesGrouped['Key Staff Application']));
        $this->execute(sprintf("UPDATE `client_categories` SET `client_type_id`=%d WHERE company_id = 1 AND client_category_abbreviation = 'BCSPN'", $arrClientTypesGrouped['Skills Post-Nomination Support']));

        $this->execute("DELETE FROM `client_categories` WHERE company_id = 1 AND client_category_name = 'northeast::reviewRequest'");
    }
