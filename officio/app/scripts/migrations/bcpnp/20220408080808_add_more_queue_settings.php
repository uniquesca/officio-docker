...
class AddMoreQueueSettings extends AbstractMigration
{
    public function up()
    {
        $this->execute("ALTER TABLE `members_queues`
            CHANGE COLUMN `queue_columns` `queue_individual_columns` TEXT NULL DEFAULT NULL AFTER `queue_member_selected_queues`,
    ...
    {
        $this->execute("ALTER TABLE `members_queues`
            CHANGE COLUMN `queue_individual_columns` `queue_columns` TEXT NULL DEFAULT NULL AFTER `queue_member_selected_queues`,
