 ...
 */
class SignXLSX implements Sign
{
    /**
     * @access private
     * @var Zip
    ...
    public function sign()
    {
        ...
        // xl/_rels/workbook.xml.rels is needed to set order of References and Transforms tag
        $wordRelsDocumentContent = $this->_docx->getFromName('xl/_rels/workbook.xml.rels');
        if (PHP_VERSION_ID < 80000) {
            $optionEntityLoader = libxml_disable_entity_loader(true);
        }
        ...
        foreach ($elementsRelationWordRelsDocumentContent as $elementRelation) {
            ...
            $filesXMLToBeAdded[] = 'xl/' . $domElementRelation->getAttribute('Target');
        }
        // sort xml alphabetic
        sort($filesXMLToBeAdded);

        // add other rels, like charts and xl/_rels/workbook.xml.rels
        for ($i = 0; $i < $this->_docx->numFiles; $i++) {
            if (preg_match('/^xl.*\.xml.rels$/', $this->_docx->getNameIndex($i))) {
    ...
    private function returnIdsRels()
    {
        ...
        for ($i = 0; $i < $this->_docx->numFiles; $i++) {
            if (preg_match('/^.*\.(rels)$/', $this->_docx->getNameIndex($i))) {
                ...
                // _rels/.rels file only needs workbook.xml Id
                if ($this->_docx->getNameIndex($i) == '_rels/.rels') {
                    $elementsRelationship = $xmlContent->xpath('//rels:Relationship[@Target="xl/workbook.xml"]');
                } else {
