...
class SignPDF implements Sign
{
     ...
     */
    public function sign($target, $optionsSignature = null, $optionsImage = null, $options = array())
    {
        ...
        $certificate = 'file://' . $this->_x509Certificate;
        $private = 'file://' . $this->_privatekey;
        ...
        if ($options['annotations']) {
            for ($i = 1; $i <= $pageCount; $i++) {
                ...
                // add an image
                if ($optionsImage && is_array($optionsImage)) {
                    if ($optionsImage['page'] == $i || ($i == $pageCount && $optionsImage['page'] < 0)) {
