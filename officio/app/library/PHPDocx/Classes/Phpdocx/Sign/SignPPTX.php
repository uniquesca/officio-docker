 ...
 */
class SignPPTX implements Sign
{
    /**
     * @access private
     * @var Zip
    ...
    public function sign()
    {
        ...
        // ppt/_rels/presentation.xml.rels is needed to set order of References and Transforms tag
        $wordRelsDocumentContent = $this->_docx->getFromName('ppt/_rels/presentation.xml.rels');
        if (PHP_VERSION_ID < 80000) {
            $optionEntityLoader = libxml_disable_entity_loader(true);
        }
        ...
        // sort order of ppt/*.xml to add
        $filesXMLToBeAdded = array();
        // add presentation.xml from _rels/.rels
        foreach ($elementsRelationRelsRelsContent as $elementRelation) {
            $domElementRelation = dom_import_simplexml($elementRelation);
            // only presentation.xml is needed
            if ($domElementRelation->getAttribute('Target') != 'ppt/presentation.xml') {
                continue;
            }
            $filesXMLToBeAdded[] = $domElementRelation->getAttribute('Target');
        }
        // add content from ppt/_rels/presentation.xml.rels
        foreach ($elementsRelationWordRelsDocumentContent as $elementRelation) {
            ...
            $filesXMLToBeAdded[] = 'ppt/' . $domElementRelation->getAttribute('Target');
        }
        // sort xml alphabetic
        sort($filesXMLToBeAdded);

        // add other rels, like charts and ppt/_rels/presentation.xml.rels
        for ($i = 0; $i < $this->_docx->numFiles; $i++) {
            if (preg_match('/^ppt.*\.xml.rels$/', $this->_docx->getNameIndex($i))) {
                ...
                $this->addReference(
                    '/' . $this->_docx->getNameIndex($i) . '?ContentType=application/vnd.openxmlformats-package.relationships+xml',
                    $digestValue,
                    'http://www.w3.org/2000/09/xmldsig#sha1',
                    true,
                    $this->_idsRels[$this->_docx->getNameIndex($i)]
                );
            }
        }
    ...
    private function returnIdsRels()
    {
        ...
        for ($i = 0; $i < $this->_docx->numFiles; $i++) {
            if (preg_match('/^.*\.(rels)$/', $this->_docx->getNameIndex($i))) {
                ...
                // _rels/.rels file only needs presentation.xml Id
                if ($this->_docx->getNameIndex($i) == '_rels/.rels') {
                    $elementsRelationship = $xmlContent->xpath('//rels:Relationship[@Target="ppt/presentation.xml"]');
                } else {
