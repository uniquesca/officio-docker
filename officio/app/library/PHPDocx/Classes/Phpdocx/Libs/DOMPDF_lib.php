...
use Phpdocx\Transform\HTMLExtended;
use tidy;
...
class FrameParser_Tree {
    ...
    {
        if (file_exists(dirname(__FILE__) . '/../Transform/HTMLExtended.php') && HTML2WordML::$htmlExtended) {
...
class PARSERHTML {
    ...
    function load_html_file($file) {
        ...
        if ($contents === false) {/*var_dump($http_response_header);*/
            throw new Exception('Issue reading: ' . $file);
        }
    ...
    protected function _process_html($filter = '*')
    {
        ...
        if (CreateDocx::$baseCSSHTML != null && file_exists(dirname(__FILE__) . '/../Transform/HTMLExtended.php')) {
...
class StyleParser {
    ...
    function munge_colour($colour) {
        ...
        } elseif (mb_strpos($colour, "hsl") !== false) {
            $i = mb_strpos($colour, "(");
            $j = mb_strpos($colour, ")");
...
class StyleParsersheet {
     ...
     * @return array
     */
    function get_font_face_data()
    {
        return $this->_font_face_data;
    }
