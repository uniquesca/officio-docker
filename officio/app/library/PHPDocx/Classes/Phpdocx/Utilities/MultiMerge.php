...
class MultiMerge
{
            ...
            if (!isset($options['mergeType']) || $options['mergeType'] == 0) {
                ...
                for ($j = 1; $j <= count($this->_secondDocxStructuralData['headers']); $j++) {
                    foreach ($this->_secondDocxStructuralData['headers'][$j] as $key => $value) {
                        ...
                        // check if there is any rels file associated with that header
                ...
                for ($j = 1; $j <= count($this->_secondDocxStructuralData['footers']); $j++) {
                    foreach ($this->_secondDocxStructuralData['footers'][$j] as $key => $value) {
                        ...
                        // check if there is any rels file associated with that footer
    ...
    public function mergeDocxAt($firstDocument, $documentArray, $finalDocument, $referenceNode, $options)
    {
        ...
        $firstDocx = new CreateDocxFromTemplate($firstDocument);

        // get template symbols
        $templateSymbols = $firstDocx->getTemplateSymbol();
        if (is_array($templateSymbols)) {
            $templateSymbolStart = $templateSymbols[0];
            $templateSymbolEnd   = $templateSymbols[1];
        } else {
            $templateSymbolStart = $templateSymbols;
            $templateSymbolEnd   = $templateSymbols;
        }

        $referenceNodeFirstDocxEnd = array(
            'occurrence' => -1,
        );
        $content                   = new WordFragment($firstDocx, 'document');
        $content->addText($templateSymbolStart . 'MERGED_DOCUMENT_PLACEHOLDER_END' . $templateSymbolEnd);
        $firstDocx->insertWordFragment($content, $referenceNodeFirstDocxEnd);

        // add a custom placeholder where the document will be merged
        $content = new WordFragment($firstDocx, 'document');
        $content->addText($templateSymbolStart . 'MERGED_DOCUMENT_PLACEHOLDER_TARGET' . $templateSymbolEnd);
        ...
        $finalDocx = new CreateDocxFromTemplate($finalDocument . $randomPrefixFile . '_merged.docx');
        // get template symbols
        $templateSymbols = $finalDocx->getTemplateSymbol();
        if (is_array($templateSymbols)) {
            $templateSymbolStart = $templateSymbols[0];
            $templateSymbolEnd   = $templateSymbols[1];
        } else {
        ...
        $referenceNodePlaceholderFollowingSiblings = array(
            'customQuery' => '//w:p[contains(., "' . $templateSymbolStart . 'MERGED_DOCUMENT_PLACEHOLDER_END' . $templateSymbolEnd . '")]/following-sibling::*',
        );
        ...
        for ($i = $queryInfo['length']; $i >= 0; $i--) {
            $referenceNodeFrom = array(
                'customQuery' => '//w:p[contains(., "' . $templateSymbolStart . 'MERGED_DOCUMENT_PLACEHOLDER_END' . $templateSymbolEnd . '")]/following-sibling::*[' . $i . ']',
            );

            $referenceNodeTo = array(
                'customQuery' => '//w:p[contains(., "' . $templateSymbolStart . 'MERGED_DOCUMENT_PLACEHOLDER_TARGET' . $templateSymbolEnd . '")]',
            );
        ...
        $referenceNodePlaceholderEnd = array(
            'type'     => 'paragraph',
            'contains' => $templateSymbolStart . 'MERGED_DOCUMENT_PLACEHOLDER_END' . $templateSymbolEnd,
        );
        $finalDocx->removeWordContent($referenceNodePlaceholderEnd);
        $referenceNodePlaceholderTarget = array(
            'type'     => 'paragraph',
            'contains' => $templateSymbolStart . 'MERGED_DOCUMENT_PLACEHOLDER_TARGET' . $templateSymbolEnd,
        );
                ...
                if (!in_array($key, $refNumberings)) {
                    ...
                    if ($numNodes->length > 0) {
                        ...
                        if ($absNumNodes->length > 0) {
                            ...
                            if ($numNodes->item(0)->getElementsByTagName('lvlOverride')->length <= 0) {
                                // no w:lvlOverride tags
                            ...
                            if ($numNodes->item(0)->getElementsByTagName('lvlOverride')->length > 0) {
                                // w:lvlOverride tags
                                $newNumNode->setAttribute('w:abstractNumId', $newAbstractNumId);
                            }
                            ...
                            // check if w:num has lvlOverride tags
                            if ($numNodes->item(0)->getElementsByTagName('lvlOverride')->length > 0) {
                                // w:lvlOverride tags
                                $newLevelOverride = $numNodes->item(0)->getElementsByTagName('lvlOverride')->item(0)->ownerDocument->saveXML($numNodes->item(0)->getElementsByTagName('lvlOverride')->item(0));
                                // lvlOverride enable, use the source abstractNumId
                                $newNum      = '<w:num w:numId="' . $value . '" xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"><w:abstractNumId w:val="' . $absNum . '" />' . $newLevelOverride . '</w:num>';
