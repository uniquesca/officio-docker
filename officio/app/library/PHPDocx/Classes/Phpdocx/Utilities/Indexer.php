...
class Indexer
{
        ...
        if (!empty($xml)) {
            ...
            foreach ($imageEntries as $imageEntry) {
                ...
                if ($drawingEntries->length == 0) {
                    $drawingEntries = $contentTargetXpath->query('//w:drawing/*[.//@r:link="' . $imageEntry->getAttribute('Id') . '"]');
                    if (empty($imageString)) {
                        if ($imageEntry->hasAttribute('TargetMode') && $imageEntry->getAttribute('TargetMode') == 'External') {
                            $imageString = file_get_contents($imageEntry->getAttribute('Target'));
                        }
                    }
                }
                ...
                if ($drawingEntries->length > 0) {
                    ...
                    if ($drawingChildNodes) {
                        foreach ($drawingChildNodes as $drawingChildNode) {
                            ...
                            if ($drawingChildNode->tagName == 'wp:docPr') {
                                if ($drawingChildNode->hasAttribute('title')) {
                                    $altTextTitleImage = $drawingChildNode->getAttribute('title');
                                }
                                if ($drawingChildNode->hasAttribute('descr')) {
                                    $altTextDescriptionImage = $drawingChildNode->getAttribute('descr');
                                }
                            }
                        }
                        ...
                        $this->documentStructure[$target]['images'][] = array(
                            ...
                            'alt_text_title'       => $altTextTitleImage,
                            'alt_text_description' => $altTextDescriptionImage,
                        );
                    }
                }
            }
    ...
    /**
     * Extract link contents from rels files
     *
     * @param string $xml XML string
     * @param string $target Content target
     */
    protected function extractLinksRels($xml, $target)
    {
        // check if the XML is not empty
        if (!empty($xml)) {
            // load XML content
            ...
            $contentXpath = new DOMXPath($contentDOM);
            $contentXpath->registerNamespace('rel', 'http://schemas.openxmlformats.org/package/2006/relationships');
            $hyperlinkEntries = $contentXpath->query('//rel:Relationship[@Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/hyperlink"]');

            // iterate charts
            foreach ($hyperlinkEntries as $hyperlinkEntry) {
                if ($hyperlinkEntry->getAttribute('TargetMode') == 'External') {
                    $this->documentStructure[$target]['links'][] = trim($hyperlinkEntry->getAttribute('Target'));
                }
            }
        }
    }
    ...
    protected function extractStyles($xml)
    {
        ...
        if ($docDefaultEntry->length > 0) {
            ...
            if ($rPrDefaultTag->length > 0 && $rPrDefaultTag->item(0)->hasChildNodes()) {
                $parserXML = xml_parser_create();
                xml_parser_set_option($parserXML, XML_OPTION_CASE_FOLDING, 0);
            ...
            if ($pPrDefaultTag->length > 0 && $pPrDefaultTag->item(0)->hasChildNodes()) {
                $parserXML = xml_parser_create();
                xml_parser_set_option($parserXML, XML_OPTION_CASE_FOLDING, 0);
        ...
        foreach ($contentTypesXml->Override as $override) {
            foreach ($override->attributes() as $attribute => $value) {
                ...
                if ($value == 'application/vnd.openxmlformats-officedocument.wordprocessingml.document.main+xml') {
                    ...
                    // extract links from rels files
                    $this->extractLinksRels($contentRels, 'body');
                    ...
                    // extract links from rels files
                    $this->extractLinksRels($contentRels, 'headers');
                    ...
                    // extract links from rels files
                    $this->extractLinksRels($contentRels, 'footers');
