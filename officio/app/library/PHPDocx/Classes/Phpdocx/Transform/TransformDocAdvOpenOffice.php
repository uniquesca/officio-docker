...
class TransformDocAdvOpenOffice extends TransformDocAdv
{
     ...
     * @return string
     */
    public function prepareDocx($source, $options, $tempDir = null)
    {
        if ($tempDir === null) {
    ...
    public function transformDocument($source, $target, $options = array())
    {
        ...
        if ($sourceExtension == 'docx') {
            ...
            $newDocx = $this->prepareDocx($source, $options, $tempDir);
            ...
            if ($options['method'] == 'script') {
                ...
                if ($filesExtensions['targetExtension'] != 'rtf' && $options['odfconverter']) {
                ...
                passthru('java -jar ' . dirname(__FILE__) . '/../../../lib/openoffice/jodconverter-2.2.2/lib/jodconverter-cli-2.2.2.jar ' . $tempDoc . ' ' . $target . $outputDebug);
            }

            CleanTemp::clean($tempDoc);
        } else {
            if ($options['method'] == 'script') {
                passthru('php ' . dirname(__FILE__) . '/../../../lib/convert.php -s ' . $source . ' -d ' . $target . $outputDebug);
            } else {
                // how to start OpenOffice in headless mode: lib/openoffice/openoffice.org3/program/soffice -headless -accept="socket,host=127.0.0.1,port=8100;urp;" -nofirststartwizard;
                passthru('java -jar ' . dirname(__FILE__) . '/../../../lib/openoffice/jodconverter-2.2.2/lib/jodconverter-cli-2.2.2.jar ' . $source . ' ' . $target . $outputDebug);
            }
        }
