...
class TransformDocAdvHTML
{
    ...
    {
        if (file_exists(dirname(__FILE__) . '/../Utilities/DOCXStructureTemplate.php') && $docxDocument instanceof DOCXStructure) {
     ...
     * @param array $options
     *  Values:
     *    'addDefaultStyles' => default as true. If true add default MS Word styles
     *    'javaScriptAtTop' => default as false. If true add JS in the head tag.
     *    'includeBlankSpacesInEmptyParagraphs' => default as false. If true add &nbsp; to empty paragraphs. Useful to keep blank paragraphs if needed.
     *    'numberingAsParagraphs' => default as false. If true add list numberings as paragraphs
    ...
    public function transform(TransformDocAdvHTMLPlugin $htmlPlugin, $options = array())
    {
        ...
        if ($this->htmlPlugin->getGenerateSectionTags()) {
            $this->html = '<' . $this->htmlPlugin->getTag('section') . ' class="' . $this->currentSectionClassName . ' ' . ($this->htmlPlugin->getExtraClass('section') == null ? '' : $this->htmlPlugin->getExtraClass(
                    'section'
                )) . '">__HEADERCONTENTSECTION__';
        }

        $this->target = 'document';
        $this->transformXml($xmlBody);
        $documentContent = $this->html;
        if (isset($options['includeBlankSpacesInEmptyParagraphs']) && $options['includeBlankSpacesInEmptyParagraphs']) {
            $documentContent = str_replace(' "></p>', ' ">&nbsp;</p>', $documentContent);
        }
        $this->html = '';
        ...
        foreach ($xml->childNodes as $childNode) {
            ...
            switch ($childNode->nodeName) {
                ...
                case 'w:r':
                    ...
                    break;
                case 'w:tab':
                    if ($childNode->parentNode->tagName != 'w:tabs') {
                        $this->transformW_TAB($childNode, $nodeClass);
                    }
                    break;
                case 'w:t':
     ...
     * @return string Styles
     */
    protected function addPprStyles($node, $type = 'paragraph')
    {
        if ($node) {
            $styles    = '';
                        ...
                        switch ($pprStyle->tagName) {
                            case 'w:ind':
                                ...
                                if ($type != 'numberingStyleParagraph') {
                                    ...
                                    // avoid adding an extra margin when left and hanging have the same values
                                    if ($pprStyle->hasAttribute('w:left') && $pprStyle->getAttribute('w:left') == $pprStyle->getAttribute('w:hanging')) {
    ...
    protected function addRprStyles($node)
    {
        if ($node) {
            ...
            // if the previous w:pPr/w:rPr style has a color style but not the current w:rPr element, set a normal font-weight
            if ($node->parentNode && $node->parentNode->tagName == 'w:p') {
                $rprParentpPrStyles = $node->parentNode->getElementsByTagNameNS('http://schemas.openxmlformats.org/wordprocessingml/2006/main', 'pPr');
                if ($rprParentpPrStyles->length > 0) {
                    $rprParentpPrrPrStyles = $rprParentpPrStyles->item(0)->getElementsByTagNameNS('http://schemas.openxmlformats.org/wordprocessingml/2006/main', 'rPr');
                    if ($rprParentpPrrPrStyles->length > 0) {
                        $rprParentpPrrPrColorStyles = $rprParentpPrStyles->item(0)->getElementsByTagNameNS('http://schemas.openxmlformats.org/wordprocessingml/2006/main', 'color');
                        if ($rprParentpPrrPrColorStyles->length > 0) {
                    ...
                    if (isset($rprStyle->tagName)) {
                        switch ($rprStyle->tagName) {
                            ...
                            case 'w:smallCaps':
                                ...
                                } else if ($rprStyle->getAttribute('w:val') == '0') {
    ...
    protected function addStyles()
    {
        ...
        if ($styles->length > 0) {
            foreach ($styles as $style) {
                ...
                $nodeClass                       = str_replace(array('-'), '_', $nodeClass);
                $this->css[$nodeClass]           = '';
                $this->css[$nodeClass . ' span'] = '';
                ...
                switch ($style->getAttribute('w:type')) {
                    ...
                    case 'paragraph':
                        ...
                        $this->css[$nodeClass] .= $this->addRprStyles($style);
                        // apply the same styles to span tags to overwrite default styles if some exist
                        $this->css[$nodeClass . ' span'] .= $this->addRprStyles($style);
                        break;
                    default:
                        break;
                }
    ...
    protected function getCellStyles($styles)
    {
        ...
        if ($elementWTcPr->length > 0) {
            ...
            if ($elementWTcBorders->length > 0) {
                if ($elementWTcBorders->item(0)->childNodes->length == 0) {
                    // no border tags, add all
                    $cellStyles       .= 'border-top: ' . $this->htmlPlugin->transformSizes(8, 'eights') . ' solid #000000;';
                    $borderStylesCell .= 'border-top: ' . $this->htmlPlugin->transformSizes(8, 'eights') . ' solid #000000;';
                    $cellStyles       .= 'border-right: ' . $this->htmlPlugin->transformSizes(8, 'eights') . ' solid #000000;';
                    $borderStylesCell .= 'border-right: ' . $this->htmlPlugin->transformSizes(8, 'eights') . ' solid #000000;';
                    $cellStyles       .= 'border-bottom: ' . $this->htmlPlugin->transformSizes(8, 'eights') . ' solid #000000;';
                    $borderStylesCell .= 'border-bottom: ' . $this->htmlPlugin->transformSizes(8, 'eights') . ' solid #000000;';
                    $cellStyles       .= 'border-left: ' . $this->htmlPlugin->transformSizes(8, 'eights') . ' solid #000000;';
                    $borderStylesCell .= 'border-left: ' . $this->htmlPlugin->transformSizes(8, 'eights') . ' solid #000000;';
                } else {
                    // top
    ...
    protected function transformM_OMATH($childNode)
    {
        ...
        $mathMLXML = '<?xml version="1.0" encoding="UTF-8" standalone="yes"?><w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main" xmlns:m="http://schemas.openxmlformats.org/officeDocument/2006/math">' . $childNode->ownerDocument->saveXML(
                $childNode
            ) . '</w:document>';
    ...
    {
        $href   = $this->getRelationshipContent($childNode->getAttribute('r:id'));
        $target = 'target="_blank"';

        if (!$href) {
            // handle TOC anchors
            if ($childNode->hasAttribute('w:anchor')) {
                $href   = '#' . $childNode->getAttribute('w:anchor');
                $target = '';
            }
        }
    ...
    protected function transformW_INSTRTEXT($childNode, $nodeClass)
    {
        ...
        if (is_array($contentComplexField) && isset($contentComplexField[0])) {
            if ($contentComplexField[0] == 'FORMCHECKBOX') {
                // get the previous sibling w:r to know if the checkbox is enabled or not
                $nodePrevInstrText = $childNode->parentNode->previousSibling;
                if ($nodePrevInstrText->tagName != 'w:r') {
                    // get the previous sibling, as a bookmark or other intermediate element has be found
                    $nodePrevInstrText = $nodePrevInstrText->previousSibling;
                }
                $xpathPInstrText = new DOMXPath($nodePrevInstrText->ownerDocument);
                $xpathPInstrText->registerNamespace('w', 'http://schemas.openxmlformats.org/wordprocessingml/2006/main');
                $defaultNodes = $xpathPInstrText->query('.//w:checkBox/w:default', $nodePrevInstrText);
    ...
    protected function transformW_P($childNode, $nodeClass)
    {
        ...
        if ($outlineLvlTag->length > 0 && $outlineLvlTag->item(0)->hasAttribute('w:val')) {
            ...
            // handle heading tag in custom styles
            ...
            if ($pStyle->length > 0) {
                ...
                $xpathStyles = new DOMXPath($this->stylesDocxDOM);
                $xpathStyles->registerNamespace('w', 'http://schemas.openxmlformats.org/wordprocessingml/2006/main');
                $outlineLvlTag = $xpathStyles->query('//w:style[@w:styleId="' . $pStyleId . '"]/w:pPr/w:outlineLvl');

                if ($outlineLvlTag->length > 0) {
                    $elementTag = $this->htmlPlugin->getTag('heading') . ((int)$outlineLvlTag->item(0)->getAttribute('w:val') + 1);
                }
            }
        }
        ...
        if (is_array($this->numberingParagraph)) {
            ...
            if ($numPrTag->length > 0) {
                ...
                if ($numIdTag->length > 0 && $numIdTag->item(0)->hasAttribute('w:val') && $numIdTag->item(0)->getAttribute('w:val') != '') {
                    ...
                    // restart the value if it's the first time it appears or the level has changed and the new level is higher than the old one
                    if (!isset($this->numberingParagraph['value'][$numberingLevel . $numIdTag->item(0)->getAttribute('w:val')]) || ($this->numberingParagraph['level'][$numIdTag->item(0)->getAttribute('w:val')] != $numberingLevel && (int)$this->numberingParagraph['level'] < (int)$numberingLevel)) {
                    ...
                    $this->numberingParagraph['level'][$numIdTag->item(0)->getAttribute('w:val')] = $numberingLevel;
        ...
        if ($pStyle->length > 0) {
            $pStyleId = $pStyle->item(0)->getAttribute('w:val');
            // clean not valid characters
            $pStyleId = str_replace(array('-'), '_', $pStyleId);
            if (!empty($pStyleId)) {
                $this->css[$nodeClass . ' .paragraph_' . $pStyleId] = '';
                $nodeClassHTML                                      = str_replace(array(',', '.'), '', $nodeClassHTML . ' .paragraph_' . $pStyleId);
            }
        }
        ...
        if (!is_array($this->numberingParagraph)) {
            ...
            if ($numPrTag->length > 0) {
                ...
                if ($nextSiblingElement !== null && $nextSiblingElement->nodeType != 3) {
    ...
    protected function transformW_R($childNode, $nodeClass)
    {
        ...
        if ($rStyle->length > 0) {
            ...
            // clean not valid characters
            $rStyleId = str_replace(array('-'), '_', $rStyleId);
            if (!empty($rStyleId)) {
                $this->css[$nodeClass . ' .character_' . $rStyleId]            = '';
                $this->css['_span.' . $nodeClass . ' .character_' . $rStyleId] = '';
                $nodeClassHTML                                                 = str_replace(array(',', '.'), '', $nodeClassHTML . ' .character_' . $rStyleId);
            }
        }
        ...
        // if there's no w:t content add a blank space to avoid empty paragraphs
        $tTags = $childNode->getElementsByTagNameNS('http://schemas.openxmlformats.org/wordprocessingml/2006/main', 't');
        if ($childNode->nodeValue === '' && $tTags->length < 1) {
            $this->html .= '&nbsp;';
        }
                    ...
                    if ($rowspanValue == 'restart') {
                        ...
                        foreach ($rowspan as $rowspanKeys => $rowspanValues) {
                            if ($rowspanKeys == '__ROWSPANVALUE__' . ($indexTr - 1) . '__' . $indexTd . '__') {
                                ...
                                if ((@(int)$rowSpanValueRow[2] - @(int)$rowSpanValueRowPrevious[2]) > 1 || (@(int)$rowSpanValueRow[2] - @(int)$rowSpanValueRowPrevious[2]) == 0) {
                                    $activeRowspan = false;
                                }
                        ...
                        if ($activeRowspan) {
                            $continue = true;
                            for ($i = 1; $i < 50; $i++) {
                                if (isset($rowspan['__ROWSPANVALUE__' . ($indexTr - $i) . '__' . $indexTd . '__']) && $maxActiveRowspan > 0 && $continue) {
                                    $rowspan['__ROWSPANVALUE__' . ($indexTr - $i) . '__' . $indexTd . '__'] += 1;
                                    $maxActiveRowspan--;
                                    if (!isset($rowspan['__ROWSPANVALUE__' . ($indexTr - $i + 1) . '__' . $indexTd . '__'])) {
                                        $continue = false;
                                    }
                                }
                            }
