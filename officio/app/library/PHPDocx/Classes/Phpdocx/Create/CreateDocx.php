...
class CreateDocx extends CreateDocument
{
            ...
            try {
                ...
                foreach ($chartNodes as $chartNode) {
                    ...
                    foreach ($chartRelsXlsxNodes as $chartRelsXlsxNode) {
                        ...
                        } elseif ($relationshipOtherTypeValue == 'http://schemas.microsoft.com/office/2011/relationships/chartStyle') {
        ...
        if (file_exists($fontSource)) {
            ...
            if ($fontQuery->length > 0) {
                ...
                if ($fontQueryEmbed->length == 0) {
                    ...
                    $fontEntry = '<w:' . $options['styleEmbedding'] . ' r:id="rId' . $idFont . '" w:fontKey="' . $odttfFont['guid'] . '"/>';
     ...
     * 'useHTMLExtended' (boolean)  if true uses HTML extended tags and CSS extended styles. Default as false
    ...
    public function embedHTML($html = '<html><body></body></html>', $options = array())
    {
        ...
        if (file_exists(dirname(__FILE__) . '/../Transform/HTMLExtended.php') && $options['useHTMLExtended'] && strpos($html, '<math') !== false) {
                ...
                if (file_exists(dirname(__FILE__) . '/../Transform/HTMLExtended.php') && $options['useHTMLExtended']) {
                    if (isset($value['lvlOverride']) && count($value['lvlOverride']) > 0) {
                        foreach ($value['lvlOverride'] as $valueLvlOverride) {
                            ...
                            $newOverrideTags       = '<w:num w:numId="' . $value['id'] . '"><w:abstractNumId w:val="' . $value['id'] . '"/></w:num><w:num w:numId="' . $valueLvlOverride['id'] . '">' . str_replace(
                                    '<w:abstractNumId w:val=""',
                                    '<w:abstractNumId w:val="' . $value['id'] . '"',
                                    CreateDocx::$customLists[$valueLvlOverride['listStyle']]['wordML']
                                ) . '</w:num></w:numbering>';
        ...
        if (file_exists(dirname(__FILE__) . '/../Transform/HTMLExtended.php') && $options['useHTMLExtended'] && count($nodesMathEquation) > 0) {
    ...
    public function importListStyle($path, $id, $name)
    {
        ...
        $listId                             = rand(999, 32767);
        $originalAbstractNumId              = rand(999, 32767);
    ...
    protected function saveToZip($src, $target, &$zip = '')
    {
        if (!is_object($src) && @is_file(str_replace(chr(0), '', $src))) {
            ...
            if (is_string($src)) {
                $XMLData = $src;
            } elseif ($src instanceof DOMDocument) {
                $XMLData = $src->saveXML();
            } elseif ($src instanceof SimpleXMLElement) {
                $XMLData = $src->asXML();
            } else {
                $XMLData = $src;
            }
