...
class CreateDocxFromTemplate extends CreateDocx
{
     ...
     * @var string
     */
    public static $_uniqueTemplateSymbol = true;
     ...
     * @var string
     */
    public static $_distinctTemplateSymbol = false;
     ...
     * @var string
     */
    public static $_templateSymbolStart = '$';
     ...
     * @var string
     */
    public static $_templateSymbolEnd = '$';
     ...
     * @var string
     */
    public static $_templateSymbolMergeFieldMode = false;
    ...
    {
        if (self::$_templateSymbolStart == self::$_templateSymbolEnd && strlen(self::$_templateSymbolStart) == 1) {
            return self::$_templateSymbolStart;
        } else {
            return array(self::$_templateSymbolStart, self::$_templateSymbolEnd);
        }
    }
    ...
    {
        // disable mergefield mode as default, if needed it's enabled in this same method
        self::$_templateSymbolMergeFieldMode = false;

        if (is_null($templateSymbolEnd)) {
            self::$_templateSymbolStart = $templateSymbolStart;
            self::$_templateSymbolEnd   = $templateSymbolStart;
        } else {
            self::$_templateSymbolStart = $templateSymbolStart;
            self::$_templateSymbolEnd   = $templateSymbolEnd;
        }

        // mergefield mode
        if ($templateSymbolStart == 'mergefield') {
            self::$_templateSymbolStart          = '«';
            self::$_templateSymbolEnd            = '»';
            self::$_templateSymbolMergeFieldMode = true;
        }
    }
    ...
    public function clearBlocks()
    {
        ...
        $name  = self::$_templateSymbolStart . self::$_templateBlockSymbol;
    ...
    public function cloneBlock($blockName, $occurrence = 1)
    {
        ...
        $contentNodesReferencedWordContent = $domXpath->query(
            '//w:p[w:r/w:t[text()[contains(.,"' . self::$_templateSymbolStart . self::$_templateBlockSymbol . $blockName . self::$_templateSymbolEnd . '")]]][' . $occurrence . ']/following-sibling::*'
        );

        $contentNodeReferencedToWordContent = $domXpath->query('//w:p[w:r/w:t[text()[contains(.,"' . self::$_templateSymbolStart . self::$_templateBlockSymbol . $blockName . self::$_templateSymbolEnd . '")]]][' . ($occurrence + 1) . ']');

        $contentNodeReferencedToWordContentParent = $domXpath->query(
            '//w:p[w:r/w:t[text()[contains(.,"' . self::$_templateSymbolStart . self::$_templateBlockSymbol . $blockName . self::$_templateSymbolEnd . '")]]][' . ($occurrence + 1) . ']/..'
        );
        ...
        if ($contentNodesReferencedWordContent->length > 0 && $contentNodeReferencedToWordContent->length > 0) {
            ...
            foreach ($contentNodesReferencedWordContent as $contentNodeReferencedWordContent) {
                if ($contentNodeReferencedWordContent->nodeValue == self::$_templateSymbolStart . self::$_templateBlockSymbol . $blockName . self::$_templateSymbolEnd) {
                    break;
                }
    ...
    public function deleteTemplateBlock($blockName)
    {
        ...
        foreach ($aType as $type) {
            ...
            $loadContent = preg_replace(
                '/\\' . self::$_templateSymbolStart . $type . $blockName . '\\' . self::$_templateSymbolEnd . '[.|\s|\S]*?\\' . self::$_templateSymbolStart . $type . $blockName . '\\' . self::$_templateSymbolEnd . '/ms',
                self::$_templateSymbolStart . $variableName . self::$_templateSymbolEnd,
                $loadContent
            );
            // use XPath to find all paragraphs that include the variable name
            $name        = self::$_templateSymbolStart . $variableName . self::$_templateSymbolEnd;
            $domDocument = new DOMDocument();
            if (PHP_VERSION_ID < 80000) {
                $optionEntityLoader = libxml_disable_entity_loader(true);
            }
    ...
    public function getTemplateVariables($target = 'all', $prefixes = array(), $variables = array())
    {
        ...
        if ($target == 'document') {
            if (self::$_templateSymbolStart == self::$_templateSymbolEnd && strlen(self::$_templateSymbolStart) == 1) {
                $documentSymbol = explode(self::$_templateSymbolStart, $this->_wordDocumentC);
            ...
            foreach ($xpathHeadersResults as $headersResults) {
                ...
                if (self::$_templateSymbolStart == self::$_templateSymbolEnd && strlen(self::$_templateSymbolStart) == 1) {
                    $documentSymbol = explode(self::$_templateSymbolStart, $loadContent);
                    $variables      = $this->extractVariables($target, $documentSymbol, $variables);
                } else {
            ...
            if (self::$_templateSymbolStart == self::$_templateSymbolEnd && strlen(self::$_templateSymbolStart) == 1) {
                $documentSymbol = explode(self::$_templateSymbolStart, $this->_wordFootnotesT->saveXML());
                $variables      = $this->extractVariables($target, $documentSymbol, $variables);
            } else {
            ...
            if (self::$_templateSymbolStart == self::$_templateSymbolEnd && strlen(self::$_templateSymbolStart) == 1) {
                $documentSymbol = explode(self::$_templateSymbolStart, $this->_wordEndnotesT->saveXML());
                $variables      = $this->extractVariables($target, $documentSymbol, $variables);
            } else {
            ...
            if (self::$_templateSymbolStart == self::$_templateSymbolEnd && strlen(self::$_templateSymbolStart) == 1) {
                $documentSymbol = explode(self::$_templateSymbolStart, $this->_wordCommentsT->saveXML());
                $variables      = $this->extractVariables($target, $documentSymbol, $variables);
            } else {
    ...
    public function replaceListVariable($variable, $listValues, $options = array())
    {
        ...
        if ($target == 'document') {
            ...
            $search = self::$_templateSymbolStart . $variable . self::$_templateSymbolEnd;
            ...
            foreach ($listValues as $listKey => $listValue) {
                if ($listValue instanceof WordFragment) {
                    ...
                    $uniqueKey                      = self::$_templateSymbolStart . $uniqueId . self::$_templateSymbolEnd;
    ...
    public function replaceTableVariable($vars, $options = array())
    {
        ...
        if ($target == 'document') {
            ...
            for ($j = 0; $j < count($varKeys); $j++) {
                $search[$j] = self::$_templateSymbolStart . $varKeys[$j] . self::$_templateSymbolEnd;
            }
            $queryArray = array();
     ...
     * 'embedFonts' (boolean) default as false. If true download and embed TTF fonts from font-face styles
    ...
    public function replaceVariableByHTML($var, $type = 'block', $html = '<html><body></body></html>', $options = array())
    {
        if (!isset($options['forceNotTidy'])) {
            $options['forceNotUsingTidy'] = false;
        }
        ...
        if (file_exists(dirname(__FILE__) . '/../Transform/HTMLExtended.php')) {
            if (isset($options['stylesReplacementType'])) {
                if ($options['stylesReplacementType'] == 'usePlaceholderStyles') {
                    ...
                    $query     = '//w:p[w:r/w:t[text()[contains(.,"' . self::$_templateSymbolStart . $var . self::$_templateSymbolEnd . '")]]]';
                    ...
                    if ($pPrStyles->length > 0) {
                        ...
                        if ($pPrStylesNode->length > 0) {
                            ...
                            $domHTMLFragment = new DOMDocument();
                            if (PHP_VERSION_ID < 80000) {
                                $optionEntityLoader = libxml_disable_entity_loader(true);
                            }
                            $domHTMLFragment->loadXML(
                                '<w:root xmlns:wpc="http://schemas.microsoft.com/office/word/2010/wordprocessingCanvas" 
                                               ...
                                               mc:Ignorable="w14 wp14">' . (string)$htmlFragment . '</w:root>'
                            );
                            if (PHP_VERSION_ID < 80000) {
                                libxml_disable_entity_loader($optionEntityLoader);
                            }
                            $domHTMLFragmentXpath = new DOMXPath($domHTMLFragment);
                            $domHTMLFragmentXpath->registerNamespace('w', 'http://schemas.openxmlformats.org/wordprocessingml/2006/main');
                            $query                 = '//w:pPr';
                            $pPrStylesHTMLFragment = $domHTMLFragmentXpath->query($query);
                            // the new content can have more than one pPr, replace all
                            if ($pPrStylesHTMLFragment->length > 0) {
                                foreach ($pPrStylesHTMLFragment as $pPrStyleHTMLFragment) {
                                    ...
                                    if ($options['stylesReplacementType'] == 'mixPlaceholderStyles') {
                                        ...
                                        foreach ($pPrStyleHTMLFragment->childNodes as $pPrStyleHTMLFragmentNode) {
                                            if (!strstr($pPrStylesXML, $pPrStyleHTMLFragmentNode->tagName)) {
                                                // don't add the style if it's ignored
                                                if (!isset($options['stylesReplacementTypeIgnore']) || (isset($options['stylesReplacementTypeIgnore']) && !in_array(
                                                            $pPrStyleHTMLFragmentNode->tagName,
                                                            $options['stylesReplacementTypeIgnore']
                                                        ))) {
                                                    $pPrStylesXML = str_replace('</w:pPr>', $pPrStyleHTMLFragmentNode->ownerDocument->saveXML($pPrStyleHTMLFragmentNode) . '</w:pPr>', $pPrStylesXML);
                                                }
                                            }
                                        }
                                    }
                                    $newPpr = $domHTMLFragment->createDocumentFragment();
                                    @$newPpr->appendXML($pPrStylesXML);
                                    $domHTMLFragment->importNode($newPpr, true);
                                    $pPrStyleHTMLFragment->parentNode->replaceChild($newPpr, $pPrStyleHTMLFragment);
                            ...
                            $newHTMLFragment = '';
                            $rootNode = $domHTMLFragment->getElementsByTagNameNS('http://schemas.openxmlformats.org/wordprocessingml/2006/main', 'root');
                            if ($rootNode->length > 0) {
                                foreach ($rootNode->item(0)->childNodes as $childNodeRoot) {
                                    $newHTMLFragment .= $childNodeRoot->ownerDocument->saveXML($childNodeRoot);
                                }

                                $htmlFragment->setWordML($newHTMLFragment);
                            }
                        }
                    }

                    // get placeholder rPr styles
                    $query     = '//w:r[w:t[text()[contains(.,"' . self::$_templateSymbolStart . $var . self::$_templateSymbolEnd . '")]]]';
                    ...
                    if ($rPrStyles->length > 0) {
                        ...
                        if ($rPrStylesNode->length > 0) {
                            ...
                            $domHTMLFragmentXpath->registerNamespace('w', 'http://schemas.openxmlformats.org/wordprocessingml/2006/main');
                            $query                 = '//w:rPr';
                            $rPrStylesHTMLFragment = $domHTMLFragmentXpath->query($query);
                            // the new content can have more than one rPr, replace all
                            if ($rPrStylesHTMLFragment->length > 0) {
                                foreach ($rPrStylesHTMLFragment as $rPrStyleHTMLFragment) {
                                    // keep original to not overwrite rPr styles in each iteration
                                    $rPrStylesXMLOriginal = $rPrStylesXML;
                                    // mix both styles
                                    if ($options['stylesReplacementType'] == 'mixPlaceholderStyles') {
                                        // get internal HTML styles
                                        foreach ($rPrStyleHTMLFragment->childNodes as $rPrStyleHTMLFragmentNode) {
                                            // don't add the style if already exits
                                            if (!strstr($rPrStylesXML, $rPrStyleHTMLFragmentNode->tagName)) {
                                                // don't add the style if it's ignored
                                                if (!isset($options['stylesReplacementTypeIgnore']) || (isset($options['stylesReplacementTypeIgnore']) && !in_array(
                                                            $rPrStyleHTMLFragmentNode->tagName,
                                                            $options['stylesReplacementTypeIgnore']
                                                        ))) {
                                                    $rPrStylesXML = str_replace('</w:rPr>', $rPrStyleHTMLFragmentNode->ownerDocument->saveXML($rPrStyleHTMLFragmentNode) . '</w:rPr>', $rPrStylesXML);
                                                }
                                            }
                                        }
                                    }
                                    $newRpr = $domHTMLFragment->createDocumentFragment();
                                    @$newRpr->appendXML($rPrStylesXML);
                                    $domHTMLFragment->importNode($newRpr, true);
                                    $rPrStyleHTMLFragment->parentNode->replaceChild($newRpr, $rPrStyleHTMLFragment);
                            ...
                            if ($rPrStylesHTMLFragment->length > 0) {
                                foreach ($rPrStylesHTMLFragment as $rPrStyleHTMLFragment) {
                                    $rPrStyleHTMLFragment->parentNode->removeChild($rPrStyleHTMLFragment);
                                }
                            }
    ...
    public function replaceVariableByText($variables, $options = array())
    {
        ...
        if (isset($options['raw']) && $options['raw'] === true) {
            foreach ($variables as $keyVariable => $valueVariable) {
                if ($target == 'document') {
                    $this->_wordDocumentC = str_replace(self::$_templateSymbolStart . $keyVariable . self::$_templateSymbolEnd, $valueVariable, $this->_wordDocumentC);
                } else if ($target == 'footnote') {
                    $newFootnotesTXML = str_replace(self::$_templateSymbolStart . $keyVariable . self::$_templateSymbolEnd, $valueVariable, $this->_wordFootnotesT->saveXML());
                    $this->_wordFootnotesT->loadXML($newFootnotesTXML);
                } else if ($target == 'endnote') {
                    $newEndnotesTXML = str_replace(self::$_templateSymbolStart . $keyVariable . self::$_templateSymbolEnd, $valueVariable, $this->_wordEndnotesT->saveXML());
                    $this->_wordEndnotesT->loadXML($newEndnotesTXML);
                } else if ($target == 'comment') {
                    $newCommentsTXML = str_replace(self::$_templateSymbolStart . $keyVariable . self::$_templateSymbolEnd, $valueVariable, $this->_wordCommentsT->saveXML());
                    ...
                    foreach ($xpathHeadersResults as $headersResults) {
                        ...
                        if (!empty($loadContent)) {
                            $newHeaderTXML                          = str_replace(self::$_templateSymbolStart . $keyVariable . self::$_templateSymbolEnd, $valueVariable, $loadContent);
                    ...
                    foreach ($xpathFootersResults as $footersResults) {
                        ...
                        if (!empty($loadContent)) {
                            $newFooterTXML                          = str_replace(self::$_templateSymbolStart . $keyVariable . self::$_templateSymbolEnd, $valueVariable, $loadContent);
    ...
    public function tickCheckboxes($variables)
    {
        ...
        foreach ($variables as $var => $value) {
            ...
            $searchTerm    = self::$_templateSymbolStart . $var . self::$_templateSymbolEnd;
    ...
    private function extractVariablesDistinctSymbols($target, $content, $variables)
    {
        ...
        foreach ($matches[0] as $variable) {
            $variables[$target][] = str_replace(array(self::$_templateSymbolStart, self::$_templateSymbolEnd), '', strip_tags($variable));
        }
    ...
    private function Image4Image($variable, $src, $loadContent, $options = array(), $rels = 'document')
    {
        ...
        $domImages = $docDOM->getElementsByTagNameNS(
        ...
        for ($i = 0; $i < $domImages->length; $i++) {
            if (
                (
                    $domImages->item($i)->getAttribute('descr') == self::$_templateSymbolStart . $variable . self::$_templateSymbolEnd ||
                    $domImages->item($i)->getAttribute('title') == self::$_templateSymbolStart . $variable . self::$_templateSymbolEnd
                ) && $imageCounter == 0) {
    ...
    private function repairVariables($variables, $content)
    {
        if (self::$_templateSymbolStart == self::$_templateSymbolEnd && strlen(self::$_templateSymbolStart) == 1) {
            ...
            $documentSymbol = explode(self::$_templateSymbolStart, $content);
            foreach ($variables as $var => $value) {
                foreach ($documentSymbol as $documentSymbolValue) {
                ...
                if (isset($preserve) && $preserve) {
                    $query  = '//w:t[text()[contains(., "' . self::$_templateSymbolStart . $var . self::$_templateSymbolStart . '")]]';
                    $docDOM = new DOMDocument();
                    if (PHP_VERSION_ID < 80000) {
                        $optionEntityLoader = libxml_disable_entity_loader(true);
                    }
    ...
    private function singleVariable4WordFragment($var, $val, $type, $loadContent, $firstMatch)
    {
        ...
        if ($val instanceof WordFragment) {
            ...
            if ($val instanceof DOCXPathResult) {
        ...
        $searchVariable = self::$_templateSymbolStart . $var . self::$_templateSymbolEnd;
        ...
        foreach ($docNodes as $node) {
            ...
            if (strpos($cleanNodeText, $searchVariable) !== false) {
                if ($type == 'block') {
                    ...
                    if (self::$_templateSymbolStart == self::$_templateSymbolEnd) {
                        $textChunks = explode(self::$_templateSymbolStart, $textNode);
                        ...
                        $newNodeText = implode(self::$_templateSymbolStart, $textChunks);
                        $newNodeText = str_replace(self::$_templateSymbolStart . '</w:t></w:r><cursorWordML/><w:r><w:t xml:space="preserve">', '</w:t></w:r><cursorWordML/><w:r><w:t xml:space="preserve">', $newNodeText);
                        $newNodeText = str_replace('</w:t></w:r><cursorWordML/><w:r><w:t xml:space="preserve">' . self::$_templateSymbolStart, '</w:t></w:r><cursorWordML/><w:r><w:t xml:space="preserve">', $newNodeText);
                    } else {
                        $newNodeText = str_replace(
                            self::$_templateSymbolStart . $var . self::$_templateSymbolEnd,
                            self::$_templateSymbolStart . '</w:t></w:r><cursorWordML/><w:r><w:t xml:space="preserve">' . self::$_templateSymbolEnd,
                            $textNode
                        );
                        $newNodeText = str_replace(self::$_templateSymbolStart . '</w:t></w:r><cursorWordML/><w:r><w:t xml:space="preserve">', '</w:t></w:r><cursorWordML/><w:r><w:t xml:space="preserve">', $newNodeText);
                        $newNodeText = str_replace('</w:t></w:r><cursorWordML/><w:r><w:t xml:space="preserve">' . self::$_templateSymbolEnd, '</w:t></w:r><cursorWordML/><w:r><w:t xml:space="preserve">', $newNodeText);
                    }
    ...
    private function variable2Text($variables, $loadContent, $options)
    {
        ...
        foreach ($variables as $var => $val) {
            $search = self::$_templateSymbolStart . $var . self::$_templateSymbolEnd;
